...
- Offline device test: cached `app_plus` must be rejected when executable/Mach-O UUID no longer matches the cached App identity.
...
1. Deploy the 2406 migration in a test/BaoTa environment; run it twice and confirm no duplicate/DDL failure, including the runtime identity index on the actual InnoDB configuration.
...
6. After obtaining `app_plus` online, break verification and change the running App identity; cached offline high privilege must not follow the spoofed/different executable or Mach-O UUID.
7. Test `scope=1`: expect `global_plus` without App-specific restriction.
8. Test multiple cards on one UDID and verify highest applicable access is selected for the current App.
9. Publish a game-update message and a generic runtime notice; verify server-supplied title/body/buttons render correctly.
10. Confirm admin rejects Bootstrap-nonempty/API-empty config; then configure valid redundant Bootstrap/API endpoints and test migration/failover/LKG without recompiling the Dylib.
11. Test temporary total network failure within offline grace, then after expiry.
12. Only after these real checks pass should `source-v2026092406` release metadata/tag be prepared.
