<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:88:"/www/wwwroot/ios_zonoeios_xyz/public/../application/admin/view/general/config/index.html";i:1789057189;s:72:"/www/wwwroot/ios_zonoeios_xyz/application/admin/view/layout/default.html";i:1583049508;s:69:"/www/wwwroot/ios_zonoeios_xyz/application/admin/view/common/meta.html";i:1583049508;s:71:"/www/wwwroot/ios_zonoeios_xyz/application/admin/view/common/script.html";i:1583049508;}*/ ?>
<!DOCTYPE html>
                        ...
                        <table class="table table-striped">
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php endif; if($index == 'uinfo'): ?>
                                    <button type="submit" onclick="sc()" class="btn btn-danger btn-embossed">生成动态库</button>
                                    <?php endif; ?>
                                </td>
                                <td></td>
                                <td></td>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-xs-12 col-sm-4">
                            <select name="row[group]" class="form-control selectpicker">
                                ...
                                <option value="<?php echo $key; ?>" <?php if(in_array(($key), explode(',',"uinfo"))): ?>selected<?php endif; ?>><?php echo $vo; ?></option>
                                <?php endforeach; endif; else: echo "" ;endif; ?>
                            </select>
                        </div>
                    </div>
...
<script>
function sc(){
    window.open('https://control.v-team.cn/panel/login');
    return false;
}
function runSystemUpdate(force){
    layer.msg('正在更新..', {
        icon: 16
        ,shade: 0.3
        ,time: false
...
function updateConfirmHtml(str){
    var changelog = (str.data && str.data.changelog) ? String(str.data.changelog) : '';
    var content = '<div style="text-align:left;line-height:1.6;"><p>' + $('<div/>').text(str.msg || '服务器有新版本').html() + '</p>';
    if (changelog) {
...
function update(){
     $.ajax({
        ...
        success: function (str) {
            if(str.code == 200){
                var incomplete = str.data && str.data.incomplete;
                layer.confirm(updateConfirmHtml(str), {
                  title: incomplete ? '更新未生效' : '发现新版本',
                  area: ['520px', 'auto'],
                  btn: [incomplete ? '关闭防篡改并重新安装' : '确认更新', '取消']
                }, function(){
                    runSystemUpdate(!!incomplete);
                });
            }else if(str.code == 204 && str.data && str.data.can_reinstall){
                var tip = updateConfirmHtml(str);
                tip += '<p style="text-align:left;color:#a94442;margin-top:8px;">若网站开了防篡改导致文件没更新成功，可重新下载安装当前版本。</p>';
                layer.confirm(tip, {
                  title: '已经是最新版本',
                  area: ['520px', 'auto'],
                  btn: ['重新下载并安装', '关闭']
                }, function(){
                    runSystemUpdate(true);
                });
            }else{
                layer.alert(str.msg)
            }
