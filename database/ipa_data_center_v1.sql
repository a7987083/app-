-- IPA Data Center v1
-- MySQL 5.7 compatible. No foreign keys by design to keep deployment/rollback simple.
...
CREATE TABLE IF NOT EXISTS `fa_ipa_scan_item` (
  ...
  KEY `idx_job_status` (`job_id`,`status`,`id`)
...
CREATE TABLE IF NOT EXISTS `fa_ipa_binary` (
  ...
  KEY `idx_sha256` (`sha256`),
  KEY `idx_type_name` (`binary_type`,`name`)
...
CREATE TABLE IF NOT EXISTS `fa_ipa_category_binding` (
  ...
  `updated_at` int unsigned NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_asset_category` (`asset_id`,`category_id`),
...
CREATE TABLE IF NOT EXISTS `fa_dylib` (
  ...
  `updated_at` int unsigned NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_dylib_key` (`dylib_key`)
...
CREATE TABLE IF NOT EXISTS `fa_dylib_version` (
  ...
  `updated_at` int unsigned NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_dylib_version_build` (`dylib_id`,`version`,`build`),
  KEY `idx_dylib_state` (`dylib_id`,`state`,`id`),
  KEY `idx_sha256` (`sha256`)
...
CREATE TABLE IF NOT EXISTS `fa_dylib_app_binding` (
  ...
  `updated_at` int unsigned NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_dylib_bundle` (`dylib_id`,`bundle_id`),
  KEY `idx_bundle_enabled` (`bundle_id`,`enabled`)
...
CREATE TABLE IF NOT EXISTS `fa_dylib_device_session` (
  ...
  KEY `idx_udid_last_seen` (`udid_hash`,`last_seen_at`),
  KEY `idx_expire_status` (`expires_at`,`status`)
...
CREATE TABLE IF NOT EXISTS `fa_dylib_nonce` (
  ...
  `created_at` int unsigned NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_nonce_hash` (`nonce_hash`),
...
CREATE TABLE IF NOT EXISTS `fa_dylib_verify_log` (
  ...
  KEY `idx_udid_created` (`udid_hash`,`created_at`),
  KEY `idx_dylib_created` (`dylib_key`,`created_at`),
  KEY `idx_result_created` (`result_code`,`created_at`)
