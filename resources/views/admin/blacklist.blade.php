...
@section('title'){{ __('黑名单列表') }}@endsection
@section('link')
...
@section('script')
    <script>
        ...
        let blacklistTableList = {
            extend: 'collection',
            text: '更多操作',
            buttons: [
                {
                    text: '清空所有', action: function () {
                        layer.confirm('您将清空所有黑名单，并且重置数据库递增？', {
                            btn: ['确定清空', '取消']
                        }, function() {
                            layer.load(2);
                            SendAjax({
                                url: systemPath+'/blacklist/delete',
                                data: {
                                    type: 'all'
                                },
                                type: 'delete',
        ...
        DataTable.on('click', '.edit-btn', function() {
            ...
            RedirectTo(systemPath+'/blacklist/edit/'+data['id']);
        });
        DataTable.on('click', '.delete-btn', function() {
            ...
            layer.confirm('您确定要删除数据 “'+data['value']+'” 吗？', {
                btn: ['确定', '取消']
            }, function() {
                layer.load(2);
                SendAjax({
