...
@section('title'){{ __('介绍列表') }}@endsection
@section('link')
...
@section('script')
    <script>
        ...
        let aboutUSTableList = {
            extend: 'collection',
            text: '更多操作',
            buttons: [
                {
                    text: '清空所有', action: function () {
                        layer.confirm('您将清空所有介绍，并且重置数据库递增？', {
                            btn: ['确定清空', '取消']
                        }, function() {
                            layer.load(2);
                            SendAjax({
                                url: systemPath+'/about_us/delete',
                                data: {
                                    type: 'all'
                                },
                                type: 'delete',
        ...
        DataTable.on('click', '.edit-btn', function() {
            ...
            RedirectTo(systemPath+'/about_us/edit/'+data['id']);
        });
        DataTable.on('click', '.delete-btn', function() {
            ...
            layer.confirm('您确定要删除介绍 “'+data['title']+'” 吗？', {
                btn: ['确定', '取消']
            }, function() {
                layer.load(2);
                SendAjax({
