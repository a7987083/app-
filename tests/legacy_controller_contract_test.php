...
foreach (array('App-mb.php', 'Index2.php') as $file) {
    if (is_file($root . '/application/index/controller/' . $file)) {
        legacyContractFail('retired legacy controller returned: ' . $file);
    }
}

foreach (array('App-mb', 'Index2') as $name) {
    if (stripos($route, $name) !== false) {
        legacyContractFail('route.php references retired legacy controller: ' . $name);
    }
    if (strpos($audit, $name) === false) {
        legacyContractFail('audit script no longer documents retired controller: ' . $name);
    }
}
