...
p143ui_assert(strpos($controller, "public function scan()") === false, 'whole-site scan endpoint must stay removed');
p143ui_assert(strpos($controller, "if (!\$this->request->isPost())") !== false, 'destructive endpoint must be POST-only');
p143ui_assert(strpos($controller, "intval(\$this->request->param('apply', 0)) === 1") !== false, 'cleanup must default to preview mode');

foreach (['ops-version', 'ops-backups', 'ops-history-count', 'ops-running-count', 'ops-storage', 'ops-preview', 'ops-clean', 'ops-jobs', 'ops-lock'] as $id) {
