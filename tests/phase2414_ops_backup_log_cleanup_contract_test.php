...
must_contain($manager, 'protectedBackupIds()', 'history-based rollback protection');
...
must_contain($opsJs, '回滚保护', 'protected backup UI label');
must_contain($opsJs, "mode:'backup_selected'", 'backup delete request mode');
must_contain($opsJs, '.ops-backup-check:not(:disabled)', 'protected backups excluded from bulk selection');
...
fwrite(STDOUT, "phase2414 ops backup/log cleanup contract: OK\n");
