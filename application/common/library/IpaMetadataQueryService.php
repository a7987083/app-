...
/**
 * Server-side metadata listing for the active IPA work-set.
 *
 * Important: count and rows MUST be built from independent Query instances.
 * Cloning a bound ThinkPHP 5/PDO query can duplicate placeholders and cause
 * PDO error 2031 when state/keyword filters are present.
 */
class IpaMetadataQueryService
{
    ...
    {
        $state=trim((string)$state);$keyword=trim((string)$keyword);
        $page=max(1,(int)$page);$limit=max(10,min(100,(int)$limit));
        $allowedSort=['id','file_name','package_name','bundle_id','package_version','parse_state','parsed_at','updatetime'];
        if($sort==='updated_at')$sort='updatetime';
        if(!in_array($sort,$allowedSort,true))$sort='id';
        $order=strtolower((string)$order)==='asc'?'asc':'desc';

        $total=(int)self::filteredQuery($state,$keyword,$referencedOnly)->count();
        $rows=self::filteredQuery($state,$keyword,$referencedOnly)->order($sort,$order)->page($page,$limit)->select();
    ...
    {
        $base=function()use($referencedOnly){$q=Db::name('ipa_metadata');if($referencedOnly)$q->where('referenced',1);return $q;};
        return [
            'total'=>(int)$base()->count(),
            'success'=>(int)$base()->where('parse_state','success')->count(),
            'pending'=>(int)$base()->where('parse_state','pending')->count(),
            'failed'=>(int)$base()->where('parse_state','failed')->count(),
        ];
    }

    protected static function filteredQuery($state,$keyword,$referencedOnly)
    {
        $query=Db::name('ipa_metadata');
        if($referencedOnly)$query->where('referenced',1);
