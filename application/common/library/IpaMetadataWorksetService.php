...
class IpaMetadataWorksetService
{
    ...
    /** Remove unreferenced, unbound work rows; never touch parsing rows. */
    public static function pruneUnreferenced($limit=10000)
    {
        $limit=max(1,min(50000,(int)$limit));$deleted=0;$cached=0;
        while($deleted<$limit){
            $ids=self::candidateIds(min(500,$limit-$deleted));if(!$ids)break;
            ...
            foreach($ids as $id)IpaMetadataPayloadStore::remove((int)$id);
            if($count===0)break;
        }
    ...
    {
        $prefix=(string)Config::get('database.prefix');$prefix=preg_replace('/[^A-Za-z0-9_]/','',$prefix);
