...
class Occodegen extends Backend
{
    ...
    public function current()
    {
        ...
        $config = $generator->currentConfig($this->request->domain());
        $config = $generator->normalize($config, $this->request->domain());
        return json([
            'code' => 200,
            'msg' => 'ok',
            'data' => [
    ...
    public function preview()
    {
        ...
        try {
            $generator = new ObjectiveCGenerator();
            $config = $generator->normalize($this->draftConfig(), $this->request->domain());
            $validation = $generator->validate($config);
            $files = [];
            if ($validation['valid']) {
    ...
    public function generate()
    {
        ...
        try {
            ...
            $validation = $generator->validate($config);
            if (!$validation['valid']) {
                return json(['code' => 422, 'msg' => implode('；', $validation['errors']), 'data' => ['validation' => $validation]]);
            }
            ...
            if ($expected === '' || !hash_equals($hash, $expected)) {
                return json(['code' => 409, 'msg' => '配置已变化，请重新预览后再生成', 'data' => ['config_hash' => $hash]]);
            }
            if (!class_exists('ZipArchive')) {
                throw new \RuntimeException('服务器未安装 ZipArchive，无法生成 ZIP');
            }
            $this->cleanupExpired();
            $dir = $this->runtimeDir();
            if (!is_dir($dir) && !@mkdir($dir, 0755, true)) {
                throw new \RuntimeException('无法创建代码生成缓存目录');
            }
            ...
            foreach ($generator->generate($config) as $path => $content) {
                $zip->addFromString('GeneratedOC/' . $path, $content);
            }
            $zip->close();

            $filename = $config['project_name'] . '-ObjectiveC-' . substr($hash, 0, 12) . '.zip';
            $meta = [
                'token' => $token,
                'filename' => $filename,
                'config_hash' => $hash,
    ...
    public function download()
    {
        ...
        if (!preg_match('/^[a-f0-9]{32}$/', $token)) {
            return json(['code' => 400, 'msg' => '下载标识无效', 'data' => '']);
        }
        $dir = $this->runtimeDir();
        $zipPath = $dir . $token . '.zip';
        $metaPath = $dir . $token . '.json';
        $meta = is_file($metaPath) ? json_decode((string)@file_get_contents($metaPath), true) : null;
        if (!is_array($meta) || !is_file($zipPath)) {
            return json(['code' => 404, 'msg' => '生成文件不存在或已过期', 'data' => '']);
        }
        ...
        if ($content === false) {
            return json(['code' => 500, 'msg' => '读取生成文件失败', 'data' => '']);
        }
    ...
    protected function draftConfig()
    {
        $raw = $this->request->post('config', '');
        if (is_array($raw)) return $raw;
        $decoded = json_decode((string)$raw, true);
        if (!is_array($decoded)) throw new \InvalidArgumentException('配置 JSON 无效');
        return $decoded;
    }
