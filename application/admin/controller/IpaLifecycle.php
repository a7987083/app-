...
class IpaLifecycle extends Backend
{
    ...
    {
        try{$this->success('',null,['stats'=>IpaGovernanceLifecycleService::stats(),'rows'=>IpaGovernanceLifecycleService::listIgnored((int)$this->request->get('limit/d',100))]);}
    ...
    {
        if(!$this->request->isPost())$this->error('Method not allowed');
        try{$ids=json_decode((string)$this->request->post('issue_ids_json','[]'),true);if(!is_array($ids))throw new RuntimeException('issue_ids_json 无效');$this->success('已恢复忽略项',null,IpaGovernanceLifecycleService::unignoreBatch($ids,(int)$this->auth->id));}
        catch(\Exception $e){$this->error($e->getMessage());}
    }
